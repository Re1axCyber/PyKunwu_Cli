...
async def list_files(dir_path):
    for root, dirs, files in os.walk(dir_path):
        for f in files:
            ...
            await upload_file(calculatefile_md5(file_text(os.path.join(root, f))), file_text(os.path.join(root, f)))
...
async def get_type(filepath):
    if os.path.exists(filepath):
        if os.path.isdir(filepath):
            ...
            await upload_file(calculatefile_md5(file_text(filepath)), file_text(filepath))
            # file_text(path)
    else:
